...
class AddMediaViewModel(
     ...
     * to which we use the [apply] extension function to add to the [ContentValues] under the key
     * [MediaStore.Images.Media.DISPLAY_NAME] the file name generated by our method [generateFilename]
     * with our [Source] parameter [source] supplying the prefix for the file, and the string "jpg"
     * as the extension of the file. Finally we return the [Uri] returned from the method
     * [ContentResolver.insert] of a [ContentResolver] instance for our application's package when
     * we call it with `imageCollection` as the URL of the table to insert into, and `newImage` as
     * the values for the newly inserted row (it creates a `newImage` file in the `imageCollection`
     * folder and returns a content [Uri] that can be used to access it).
     *
