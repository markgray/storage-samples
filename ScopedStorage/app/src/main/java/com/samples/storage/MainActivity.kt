...
class MainActivity : AppCompatActivity() {
     ...
     * our layout file `R.layout.activity_main`. It consists of a [ConstraintLayout] root view
     * holding a single `fragment` container whose "android:name" attribute specifies that a
     * [NavHostFragment] will be instantiated in it.
     *
     * We initialize our [ConstraintLayout] variable `rootView` to the view with ID `R.id.root_view`
     ...
     *
     * We initialize our [NavController] variable `val navController` by finding the
     * [NavHostFragment] with the resource ID `R.id.nav_host_fragment` in our UI. We initialize our
     * [AppBarConfiguration] field [appBarConfiguration] to a new instance constructed to use the
     * navigation graph of `navController` (the file navigation/nav_graph.xml) for its "UP" behavior
     * (its start destination ([MainFragment]) will be considered the only top level destination,
     * and the "Up" button will not be displayed when on the start destination of the graph).
     * Finally we call the [setupActionBarWithNavController] method with `navController` as the
     * [NavController] whose navigation actions will be reflected in the title of the action bar,
     * and [appBarConfiguration] for additional configuration options for customizing the behavior
     * of the `ActionBar`.
     *
